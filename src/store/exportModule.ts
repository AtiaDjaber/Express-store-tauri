...
class ExportModule extends VuexModule {
    ...
    addItem(sale: Transfer) {
        ...
        const index = this.facture.transfers.findIndex((e) => e.product_id == productNew.product_id);
        if (index >= 0)
            this.facture.transfers[index].quantity = this.facture.transfers[index].quantity + 1;
