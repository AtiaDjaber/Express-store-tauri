<template>
  <v-dialog v-model="dialog" persistent max-width="450">
    ...
    <v-card v-if="dialog">
      ...
      <v-divider color="#875A7B"></v-divider>
      <v-divider color="#875A7B"></v-divider>
      <v-card-title style="color: gray" class="text-h4">تعديل الكمية</v-card-title>
      <v-card-text>
        <v-text-field
            outlined
            label="الكمية"
...
export default class ReturnDialog extends Vue {
  ...
  created(){
    this.copySale=Object.assign({},this.sale);

  }
  manage() {
    this.saleApi
        .returnSale(this.copySale)
        .then((result) => {
          this.$root.$emit("returnedSale", this.copySale.facture_id);
          this.$root.$emit("updatedClient", result['data']);
